...
public class Bot {
    ...
    public Command run() {
        ...
        if (blocks.contains(Terrain.MUD) || blocks.contains(Terrain.WALL) || TruckInLane(myCar.position.lane, myCar.position.block + 1, myCar.speed)) {
            if (myCar.position.lane == 1) {
                List<Object> rightblocks = getBlocksInFront(myCar.position.lane + 1, myCar.position.block - 1, myCar.speed - 1);

                if (!(rightblocks.contains(Terrain.MUD) || rightblocks.contains(Terrain.WALL) || TruckInLane(myCar.position.lane, myCar.position.block + 1, myCar.speed))) {
                    return TURN_RIGHT;
                }
                if (hasPowerUp(PowerUps.LIZARD, myCar.powerups) && (blocks.get(myCar.speed - 1) != Terrain.WALL)) {
                    return LIZARD;
                }
                if (TruckInLane(myCar.position.lane, myCar.position.block + 1, myCar.speed)) {
                    return TURN_RIGHT;
                }
                ...
                if (!(leftblocks.contains(Terrain.MUD) || leftblocks.contains(Terrain.WALL) || TruckInLane(myCar.position.lane, myCar.position.block + 1, myCar.speed))) {
                    return TURN_LEFT;
                }
                if (hasPowerUp(PowerUps.LIZARD, myCar.powerups) && (blocks.get(myCar.speed - 1) != Terrain.WALL)) {
                    return LIZARD;
                ...
                if (TruckInLane(myCar.position.lane, myCar.position.block + 1, myCar.speed)) {
                    if (!leftblocks.contains(Terrain.WALL)) {
                        return TURN_LEFT;
                    }
                    if (!rightblocks.contains(Terrain.WALL)) {
                        return TURN_RIGHT;
                    }
                    return TURN_LEFT;
                }
    ...
    private boolean TruckInLane(int lane, int block, int speed) {
        List<Lane[]> map = gameState.lanes;
        List<Object> blocks = new ArrayList<>();
        int startBlock = map.get(0)[0].position.block;
